...
import org.eethar.platform.app.config.MailConfig;
...
import org.eethar.platform.core.rest.dto.KeyAndPasswordDTO;
...
//
//        return userFacade.findByLogin(managedUserDTO.getLogin().toLowerCase())
//                .map(user -> Response.status(BAD_REQUEST).type(TEXT_PLAIN).entity("login already in use").build())
//                .orElseGet(() -> userFacade.getRepo().findByEmail(managedUserDTO.getEmail())
...
//        Optional<User> existingUser = userFacade.getRepo().findByEmail(userDTO.getEmail());
...
//        }
//        return userFacade.findByLogin(securityUtils.getCurrentUserLogin())
//                .map(u -> {
